...
function AndroidArchitecture({ progressRef }) {
  ...
  const palette = useMemo(
    () => ({
      android: new THREE.Color("#a4c639"), // Classic Android Green
      androidHighlight: new THREE.Color("#bce142"),
  ...
  const materials = useMemo(
    () => ({
      ...
      eye: new THREE.MeshStandardMaterial({
        color: "#ffffff", // White eyes like the reference background cutout
        roughness: 0.4,
      }),
  ...
  const parts = useMemo(
    () => ({
      head: {
        from: { position: [0, 0.68, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
      ...
      face: {
        from: { position: [0, 0.65, 0.45], rotation: [0, 0, 0], scale: [0, 0, 0] }, // Hidden initially
        to: { position: [0, 0.17, -0.92], rotation: [-Math.PI / 2, 0, 0], scale: [0.82, 0.02, 0.02] },
      },
      eyeLeft: {
        from: { position: [-0.22, 0.85, 0.44], rotation: [0, 0, 0], scale: [1, 1, 1] },
        to: { position: [-0.26, 0.18, -1.12], rotation: [0, 0, 0], scale: [0.6, 0.6, 0.6] },
      },
      eyeRight: {
        from: { position: [0.22, 0.85, 0.44], rotation: [0, 0, 0], scale: [1, 1, 1] },
        to: { position: [0.26, 0.18, -1.12], rotation: [0, 0, 0], scale: [0.6, 0.6, 0.6] },
      ...
      antennaLeft: {
        from: { position: [-0.22, 1.28, 0], rotation: [0, 0, 0.4], scale: [1, 1, 1] },
        to: { position: [-0.42, 0.16, -1.18], rotation: [0, 0, Math.PI / 2], scale: [1.3, 1.1, 1.3] },
      },
      antennaRight: {
        from: { position: [0.22, 1.28, 0], rotation: [0, 0, -0.4], scale: [1, 1, 1] },
        to: { position: [0.42, 0.16, -1.18], rotation: [0, 0, Math.PI / 2], scale: [1.3, 1.1, 1.3] },
      ...
      upperArmLeft: {
        from: { position: [-0.68, 0.1, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
        to: { position: [-0.86, 0.09, -0.08], rotation: [0, 0, 0], scale: [1.4, 0.16, 1.5] },
      },
      forearmLeft: {
        from: { position: [-0.68, 0.1, 0], rotation: [0, 0, 0], scale: [0.001, 0.001, 0.001] },
        to: { position: [-1.22, 0.1, 0.18], rotation: [0, 0, 0], scale: [1.3, 0.16, 1] },
      },
      upperArmRight: {
        from: { position: [0.68, 0.1, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
        to: { position: [0.86, 0.09, -0.08], rotation: [0, 0, 0], scale: [1.4, 0.16, 1.5] },
      ...
      legLeft: {
        from: { position: [-0.22, -0.7, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
        to: { position: [-0.3, 0.08, 0.98], rotation: [0, 0, 0], scale: [1.6, 0.18, 1.5] },
      },
      legRight: {
        from: { position: [0.22, -0.7, 0], rotation: [0, 0, 0], scale: [1, 1, 1] },
        to: { position: [0.3, 0.08, 0.98], rotation: [0, 0, 0], scale: [1.6, 0.18, 1.5] },
  ...
  useFrame(({ camera, clock }, delta) => {
    ...
    materials.eye.opacity = 1 - layout; // Fade out eyes during transform
    materials.eye.transparent = true;

    if (rootRef.current) {
      rootRef.current.position.set(0, idleAmount * Math.sin(time * 1.2) * 0.035, 0);
      rootRef.current.rotation.set(
        THREE.MathUtils.lerp(0, 0, topView),
        idleAmount * Math.sin(time * 0.35) * 0.18 + THREE.MathUtils.lerp(0, 0, topView),
        THREE.MathUtils.lerp(0, 0, topView),
      );
    }
    ...
    if (shadowRef.current) {
      shadowRef.current.scale.setScalar(THREE.MathUtils.lerp(1.05, 1.18, layout));
      shadowRef.current.material.opacity = THREE.MathUtils.lerp(0.14, 0.08, layout);
    }

    // Camera transitions from front straight-on to top-down isometric
    camera.position.set(
      THREE.MathUtils.lerp(0, 0, topView),
      THREE.MathUtils.lerp(0.28, 8.2, topView),
      THREE.MathUtils.lerp(5.65, 0.04, topView),
    );
    camera.fov = THREE.MathUtils.lerp(38, 30, topView);
    camera.lookAt(
      0,
      THREE.MathUtils.lerp(0.2, 0.02, topView),
      0
    );
  ...
  return (
    <group ref={rootRef}>
      <mesh ref={shadowRef} material={materials.shadow} position={[0, -1.05, 0.08]} rotation={[-Math.PI / 2, 0, 0]}>
      ...
      {/* White/Background colored eyes perfectly positioned */}
      <mesh ref={eyeLeftRef} material={materials.eye}>
        <sphereGeometry args={[0.045, 16, 16]} />
      </mesh>
      <mesh ref={eyeRightRef} material={materials.eye}>
        <sphereGeometry args={[0.045, 16, 16]} />
      </mesh>
      ...
      <mesh ref={upperArmLeftRef} material={materials.body}>
        <capsuleGeometry args={[0.13, 0.65, 16, 24]} />
      </mesh>
      ...
      <mesh ref={legLeftRef} material={materials.body}>
        <capsuleGeometry args={[0.12, 0.35, 16, 24]} />
      </mesh>
      ...
      <group ref={lineGroupRef} visible={false}>
        ...
        <mesh material={materials.line} position={[-0.32, 0, -0.78]}>
          <boxGeometry args={[0.38, 0.018, 0.022]} />
        </mesh>
        <mesh material={materials.line} position={[0.32, 0, -0.78]}>
          <boxGeometry args={[0.38, 0.018, 0.022]} />
        </mesh>
        <mesh material={materials.line} position={[-0.3, 0, 0.98]}>
          <boxGeometry args={[0.025, 0.018, 0.48]} />
        </mesh>
        <mesh material={materials.line} position={[0.3, 0, 0.98]}>
          <boxGeometry args={[0.025, 0.018, 0.48]} />
        </mesh>
...
export default function TransformingDevice({ progressRef }) {
  return (
    <Canvas
      camera={{ position: [0, 0.28, 5.65], fov: 38 }}
      dpr={[1, 1.35]}
      gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }}
      style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}
    >
      <ambientLight intensity={0.85} />
      <directionalLight position={[3.5, 4.8, 4.2]} intensity={2.0} />
      <directionalLight position={[-3.4, 2.4, -2.6]} intensity={0.7} color="#bcecff" />
      <spotLight position={[0.3, 5, 2.2]} angle={0.42} penumbra={0.75} intensity={1.2} color="#fff" />
      <AndroidArchitecture progressRef={progressRef} />
      <ContactShadows position={[0, -1.12, 0]} opacity={0.24} scale={5.5} blur={2.8} far={4} />
    </Canvas>
  );
}
